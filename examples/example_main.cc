...
int main() {
  try {
    // Инициализация
    EngineConfig config{.project_key = "doc-system",
                        .strict_mode = true,
...
- `{{param}}` ({{type}}): {{desc}})"))
    ...
    auto endpoint_pub =
        engine.publish_block(std::move(endpoint_draft), Version{1, 0});
...
> Текущий план: {{plan}})"))
    ...
    auto limit_pub =
        engine.publish_block(std::move(limit_draft), Version{1, 0});
    ...
    // ✅ Используем только add_block_ref с BlockRef, содержащим конкретную
    // версию
    auto comp_draft =
...
**Версия:** {{version}} | **Автор:** {{author}} | **Статус:** {{status}})"))
    ...
    auto header_pub_v2 =
        engine.publish_block(std::move(header_v2), Version{2, 0});
    ...
    auto new_comp =
        CompositionDraftBuilder("docs.users.create.v2")
            .with_project_key("api-docs")
            .add_block_ref(BlockRef("doc.header", Version{2, 0},
                                    {// Явно v2.0!
