...
void print_render_result(const Result<RenderResult>& result) {
    ...
    std::cout << std::format("✅ Success | Composition: {} v{}.{} | Blocks used: {}\n",
...
int main() {
    try {
        ...
        std::cout << "=== Advanced TextFoundry Example: LLM Code Review Prompt ===\n\n";
...
**Author:** {{author}} | **Date:** {{date}} | **Audience:** {{audience}})"))
        ...
        if (!header_pub.has_value()) { std::cerr << "Header error: " << header_pub.error().message << "\n"; return 1; }
        Version header_version = header_pub.value().version();

        // 2. Role system
        auto system_draft = BlockDraftBuilder("role.system")
            .with_type(BlockType::Role)
            .with_template(Template(R"(You are a {{role_type}} assistant specialized in {{domain}}. 
Follow all instructions precisely. )"))
            ...
            .build();
        auto system_pub = engine.publish_block(std::move(system_draft), Version{1, 0});
        Version system_version = system_pub.value().version();

        // 3. Constraint chain-of-thought
        auto cot_draft = BlockDraftBuilder("constraint.cot")
...
> 4. Output in {{format}})"))
        ...
        auto json_draft = BlockDraftBuilder("constraint.json")
            .with_type(BlockType::Constraint)
            .with_template(Template(R"(**Output format:** Valid JSON only matching this schema:
```json
{
  "issues": [{{schema}}],
  "score": {{score_range}},
...
**Key features:** {{features}})"))
            ...
            .build();
        auto project_pub = engine.publish_block(std::move(project_draft), Version{1, 0});
        Version project_version = project_pub.value().version();

        // 6. Role code reviewer
        auto reviewer_draft = BlockDraftBuilder("role.reviewer")
            .with_type(BlockType::Role)
            .with_template(Template(R"(Act as a senior {{level}} code reviewer with 15+ years experience.
Review for: {{criteria}}. 
Be constructive and precise.)"))
            ...
            .build();
        auto reviewer_pub = engine.publish_block(std::move(reviewer_draft), Version{1, 0});
...
> Flag ALL potential issues!)"))
            ...
            .build();
        auto security_pub = engine.publish_block(std::move(security_draft), Version{1, 0});
        Version security_version = security_pub.value().version();

        // 8. Domain coding guidelines
        auto guidelines_draft = BlockDraftBuilder("domain.guidelines")
            .with_type(BlockType::Domain)
...
- {{additional_rules}})"))
            ...
            .build();
        auto guidelines_pub = engine.publish_block(std::move(guidelines_draft), Version{1, 0});
...
**Issues:** {{issues}})"))
            ...
            .build();
        auto bad_ex_pub = engine.publish_block(std::move(bad_ex_draft), Version{1, 0});
...
**Why good:** {{why_good}})"))
            ...
            .build();
        auto good_ex_pub = engine.publish_block(std::move(good_ex_draft), Version{1, 0});
...
**Blocks used:** {{block_count}})"))
        ...
        StructuralStyle structural{
            ...
            .preamble = "Full LLM Code Review Prompt (TextFoundry Advanced Demo)\n\n",
            .delimiter = "\\n---\\n"
        };
        StyleProfile style{.structural = structural};

        auto comp_builder = CompositionDraftBuilder("advanced.code_review_prompt")
            .with_project_key("demo")
            .with_description("Advanced LLM code review prompt using 12 reusable blocks")
            .with_style_profile(style)

            // Header
            .add_block_ref(BlockRef("meta.header", header_version, {
...
Review the provided C++ code for a templating engine.)")
            ...
            // Reviewer role
            .add_block_ref(BlockRef("role.reviewer", reviewer_version, {
                {"level", "staff engineer"},
                {"criteria", "architecture, security, performance, maintainability"}
            }))

            // Project context
            .add_block_ref(BlockRef("domain.project", project_version, {
                {"name", "TextFoundry"},
            ...
            // Constraints
            .add_block_ref(BlockRef("constraint.cot", cot_version, {
            ...
            .add_block_ref(BlockRef("constraint.security", security_version, {
                {"custom_checks", "buffer overflows, race conditions, injection in templates"}
            }))
...
```
            ...
            // Examples
            .add_block_ref(BlockRef("examples.bad", bad_ex_version, {
                {"code", R"(int* ptr = new int; delete ptr; // leak if exception)"},
                {"issues", "manual memory, no RAII"}
            }))
            .add_block_ref(BlockRef("examples.good", good_ex_version, {
                {"code", R"(std::unique_ptr<int> ptr = std::make_unique<int>(42); )"},
        ...
        if (!comp_pub.has_value()) { std::cerr << "Comp error: " << comp_pub.error().message << "\n"; return 1; }
        ...
        auto result = engine.render("advanced.code_review_prompt", comp_version, ctx);
        ...
        std::cout << "\n✅ Advanced example complete! Generated a full LLM prompt using 12 blocks.\n";
        return 0;
