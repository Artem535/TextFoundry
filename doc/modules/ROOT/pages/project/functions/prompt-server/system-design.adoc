...
* future remote client layer
  Вне server runtime, включает C++ и Python clients к одному server contract.

* local bindings layer
  Вне server runtime, включает local C++ and Python surfaces поверх того же
  engine semantics.
...
* `Python local API`
  local engine access with Python-idiomatic form.

* `C++ remote client`
  server access over remote contract.

* `Python remote client`
  server access over the same remote contract.

Server design не должен предполагать, что Python является единственным или
основным удалённым клиентом.
...
* возможность использовать один и тот же published prompt через несколько типов
  клиентов и из двух language surfaces.
