...
* remote runtime contract должен быть одинаково пригоден для C++ и Python
  clients.
...
* создать server boundary, поверх которой позже можно строить Python SDK или
  C++/Python client layers;
* сохранить симметричную multi-language story: local engine surfaces и remote
  server surfaces.
...
* вызывать один и тот же published prompt из Python, C++ backend и agent clients
  по единому API.
...
* API позволяет безопасно строить как Python, так и C++ remote clients поверх
  того же server boundary.
