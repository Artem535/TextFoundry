...
//  - Never swallow all events while a modal is visible (FTXUI needs internal events).
...
namespace tf {
  namespace {
    ...
    ftxui::Element make_header(std::string text, ftxui::Color c = ftxui::Color::White) {
      return ftxui::text(std::move(text)) | ftxui::bold | ftxui::color(c) | ftxui::center;
    }
    ...
    // Exit shortcuts. F10 mapping can vary between terminals; Esc and 'q' are reliable.
    ...
    // Spaces around keys are trimmed; values are kept as-is (except surrounding spaces).
  ...
  ftxui::Component Tui::main_layout(auto &screen) {
    ...
    auto layout = ftxui::Renderer(container, [tab_toggle, tab_container, this] {
      return ftxui::vbox({
                 ftxui::text("TextFoundry") | ftxui::bold | ftxui::center,
                 ftxui::separator(),
                 tab_toggle->Render(),
                 ftxui::separator(),
  ...
  ftxui::Component Tui::blocks_tab() {
    ...
    auto update_details = [this] {
      ...
      if (result.has_value()) {
        ...
        details_text_ = fmt::format(
            "ID: {}\nVersion: {}.{}\nType: {}\nTemplate: {}",
            b.id(), b.version().major, b.version().minor, static_cast<int>(b.type()),
            b.templ().content());
      } else {
    ...
    content = content | ftxui::CatchEvent([this, update_details](const ftxui::Event &e) {
    ...
    return ftxui::Renderer(content, [list, details] {
      auto left = ftxui::window(ftxui::text(" Blocks "), list->Render() | ftxui::flex) |
  ...
  ftxui::Component Tui::compositions_tab() {
    ...
    return ftxui::Renderer(root, [] {
      return ftxui::vbox({
               ...
               ftxui::text("Diagnostics mode: component content disabled") | ftxui::dim,
             }) |
