 ...
 public:
  ...
  Block createAndPublishBlock(const BlockId& id, const std::string& templateStr,
    ...
    BlockDraft draft = BlockDraftBuilder(id)
                           ...
                           .build();
    auto pubResult =
        engine.publish_block(std::move(draft), Engine::VersionBump::Minor);
  ...
  Composition createAndPublishComposition(
    ...
    auto pubResult = engine.publish_composition(std::move(draft),
...
TEST_SUITE("BlockDraftBuilder") {
  TEST_CASE("build block with builder") {
    ...
    auto pubBlock =
        fixture.engine
            .publish_block(std::move(draft), Engine::VersionBump::Minor)
            .value();
    auto block =
        fixture.engine.load_block("greeting.hello", pubBlock.version()).value();
...
TEST_SUITE("CompositionDraftBuilder") {
  TEST_CASE("build composition with builder") {
    ...
    auto helloPub =
        fixture.engine
            .publish_block(std::move(helloDraft), Engine::VersionBump::Minor)
            .value();
    ...
    auto pubComp =
        fixture.engine
            .publish_composition(std::move(draft), Engine::VersionBump::Minor)
            .value();
    auto comp =
        fixture.engine.load_composition("welcome.message", pubComp.version())
            .value();
...
TEST_CASE_FIXTURE(EngineTestFixture, "publish block workflow") {
  BlockDraft draft =
      BlockDraftBuilder("test.block").with_template(Template("Hello!")).build();
...
TEST_CASE_FIXTURE(EngineTestFixture, "create and save composition") {
  ...
  auto pub =
      engine.publish_composition(std::move(draft), Version{1, 0}).value();
